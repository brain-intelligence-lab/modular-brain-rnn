...
def load_n_mnist(distributed=False, T=10, data_dir = 'n_mnist'):
    train_set = n_mnist.NMNIST(data_dir, train=True, data_type='frame', frames_number=T)
    test_set = n_mnist.NMNIST(data_dir, train=False, data_type='frame', frames_number=T)
    if distributed:
        train_sampler = torch.utils.data.distributed.DistributedSampler(train_set)
        test_sampler = torch.utils.data.distributed.DistributedSampler(test_set)
    else:
...
if __name__ == '__main__':
    ...
    train_set, test_set, _, _ = load_n_mnist(distributed=False, T=10, data_dir = 'n_mnist')